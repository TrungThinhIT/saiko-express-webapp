    ...
    <div class="card fix-overflow">
        ...
        <div class="card-body custom-background set-overflow">
            <div class="row align-items-center">
                ...
                <div class="col-md-1">
                    <button type="button" class="btn btn-secondary form-control text-center " id="reset_filter">reset</button>
                </div>
                <div class="col-md-1">
            ...
            <table class="table table-striped set-border-radius">
                ...
                <tbody id="list-contracts">
                    ...
                    <tr class="text-center addHover detail-contract" data-id="{{ $item['id'] }}">
                        ...
                        <td>
                            <span class="p-2 rounded {{ $item['closed'] ? 'bg-danger' : 'bg-info' }}">{{ $item['closed'] ? 'Đã đóng' : 'Chưa đóng' }}
                            </span>
                        </td>
                    </tr>
                    @endforeach
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.detail-contract', function(event) {
            ...
            await verifyToken();
            var url = url + '?user_id=' + localStorage.getItem('user_id') + '&idToken=' + localStorage.getItem('firebase_token');
    ...
    async function fetch_data_contracts(page) {
        await verifyToken();
        $.ajax({
            type: 'GET',
            url: "{{ route('contract.index') }}",
